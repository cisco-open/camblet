---
title: 'Try Camblet out in simple environment'
...
Since cURL is not running continuously like the server, we have to use a "dummy" command to force it to run as long as we can gather the PID. 
...
Let's ignore the labels part for now; it is meant for more advanced configuration. The addresses part tells Camblet which outgoing connections require mTLS. Let's create our own services.yaml where we place the nginx server's service IP and port number.
