...
exports.sleep = exports.register = void 0;
const embedded = require("@volar/language-service");
const vscode = require("vscode-languageserver");
const protocol_1 = require("../../protocol");
const types_1 = require("../../types");
function register(connection, workspaces, initParams, initOptions, semanticTokensLegend, runtime, documents) {
    let lastCompleteUri;
    let lastCompleteLs;
    let lastCodeLensLs;
        ...
        return worker(params.textDocument.uri, token, async (service) => {
            ...
            if (!initOptions.fullCompletionList && document) {
                list.items = list.items.filter(item => {
                    const range = item.textEdit ? (vscode.InsertReplaceEdit.is(item.textEdit)
                        ? item.textEdit.replace
                        : item.textEdit.range) : list.itemDefaults?.editRange ? (vscode.Range.is(list.itemDefaults.editRange)
                        ? list.itemDefaults.editRange
                        : list.itemDefaults.editRange.replace) : undefined;
                    if (range) {
    ...
    connection.languages.diagnostics.on(async (params, token, _workDoneProgressReporter, resultProgressReporter) => {
        const result = await worker(params.textDocument.uri, token, service => {
            ...
            const mode = initOptions.serverMode === types_1.ServerMode.PartialSemantic ? 'semantic'
                : initOptions.serverMode === types_1.ServerMode.Syntactic ? 'syntactic'
                    : 'all';
